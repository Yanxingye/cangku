<template lang="pug">
  .pages-list
    .bb-nav
      ...
      a-button.fr(type="primary" icon="plus" @click="$router.push({name:'add-pages'})") 新建页面
    .bb-content
      a-form.pages-list-filter.clearfix(
        :form="form"
        ...
        a-form-item.filter-item.fr.clearfix(
          ...
          //-   icon="plus"
          //-   @click="$router.push({name:'add-pages'})").fr 新建页面
      .pages-list-table
        a-table(
          :columns="columns"
          ...
          span(slot="action"
            ...
            span.look(@click="$router.push({name:'pages-details',query:{id:record.id}})") 查看
            span.edit(@click="$router.push({name:'edit-pages',query:{id:record.id}})") 编辑
...
export default {
  ...
  methods: {
    ...mapActions(['query_page', 'modify_page_status']),
